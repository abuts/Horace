...
% Build map connecting pixel run id-s(ixeper_ids) with ixexper_id-s in IX_experiment headers
...
if numel(unique_runid) ~= obj.n_runs || any(isnan(unique_runid))
    ...
    for i=1:nruns
        ...
        % exp(i).run_id = i; but ixeper_ids and id
...
fm = fast_map(uint32(ixeper_ids),id);

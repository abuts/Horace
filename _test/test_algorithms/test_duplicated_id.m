classdef test_duplicated_id < TestCase
    ...
    methods
        ...
        function test_update_duplicates_no_empty(~)
            ...
            assertEqual(ids,runids);
            eids = cellfun(@(x)(x.exper_id),rf_proc,'UniformOutput',true);            
            assertEqual(eids,1:9);            
        end
        ...
        function test_update_duplicates(~)
            ...
            ids = cellfun(@(x)get_id(x,'run_id'),rf_proc,'UniformOutput',true);
            assertEqual(ids,[1,0,2,2,0,3,3,3]);
            eids = cellfun(@(x)get_id(x,'exper_id'),rf_proc,'UniformOutput',true);            
            assertEqual(eids,[1,0,3,4,0,6,7,8]);            
        end

        function test_do_nothing_if_data_unique_and_inverse_order(~)
            % The test is needed for cases when gen_sqw is build in append
            % mode so that initial nxspe files may be missing.
            % 
            ...
            assertEqual(rf_proc{1}.exper_id,1)
            assertEqual(rf_proc{4}.exper_id,4)            
        end
